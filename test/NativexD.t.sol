...
contract NativexDTest is BaseERC20xDTestHelper {
    ...
    function test_liquidityCap_setByOwner() public {
        ...
        vm.prank(owner);
        vm.expectEmit(true, false, false, true);
        emit INativexD.LiquidityCapUpdated(newCap);
    ...
    function test_liquidityCap_revertNonOwner() public {
        ...
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableUnauthorizedAccount.selector, alice));
        wrapped.setLiquidityCap(100 ether);
    }
    ...
    function test_liquidityCap_enforcedOnWrap() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                INativexD.LiquidityCapExceeded.selector,
                alice,
                cap, // current wrapped
                1 ether, // attempted amount
                cap // cap
            )
        );
        wrapped.wrap{ value: 1 ether }(alice, "");
    }

    function test_liquidityCap_multipleUsers() public {
        NativexD wrapped = NativexD(payable(address(erc20s[0])));
        uint256 cap = 15 ether;

        vm.prank(owner);
        wrapped.setLiquidityCap(cap);

        // Alice wraps 10 ether
        vm.prank(alice);
        wrapped.wrap{ value: 10 ether }(alice, "");
        assertEq(wrapped.wrappedAmount(alice), 10 ether);

        // Bob wraps 15 ether (his own cap)
        vm.prank(bob);
        wrapped.wrap{ value: 15 ether }(bob, "");
        assertEq(wrapped.wrappedAmount(bob), 15 ether);

        // Alice can still wrap 5 ether more
        vm.prank(alice);
        wrapped.wrap{ value: 5 ether }(alice, "");
        assertEq(wrapped.wrappedAmount(alice), 15 ether);

        // But not more
        vm.prank(alice);
        vm.expectRevert(abi.encodeWithSelector(INativexD.LiquidityCapExceeded.selector, alice, 15 ether, 1 ether, cap));
        wrapped.wrap{ value: 1 ether }(alice, "");
    }

    function test_liquidityCap_unwrapReducesWrappedAmount() public {
        NativexD wrapped = NativexD(payable(address(erc20s[0])));
        uint256 cap = 20 ether;
        uint256 wrapAmount = 15 ether;

        vm.prank(owner);
        wrapped.setLiquidityCap(cap);

        // Alice wraps 15 ether
        vm.prank(alice);
        wrapped.wrap{ value: wrapAmount }(alice, "");
        ...
        // Alice unwraps 5 ether
        uint256 fee = wrapped.quoteTransfer(alice, uint128(GAS_LIMIT));
        vm.prank(alice);
        bytes memory data = abi.encode(uint128(GAS_LIMIT), alice);
        wrapped.unwrap{ value: fee }(alice, 5 ether, data, "");

        // Simulate the cross-chain callback to execute the pending transfer
        vm.prank(address(gateways[0]));
        wrapped.onRead(abi.encode(int256(5 ether)), abi.encode(uint256(1)));

        // Wrapped amount should be reduced
        assertEq(wrapped.wrappedAmount(alice), 10 ether);

        // Alice should have received ETH back (minus the fee)
        assertGt(alice.balance, aliceBalanceBefore);

        // Alice can now wrap 10 ether more (up to cap)
        vm.prank(alice);
        wrapped.wrap{ value: 10 ether }(alice, "");
        assertEq(wrapped.wrappedAmount(alice), 20 ether);
    }
    ...
    function test_liquidityCap_zeroMeansUnlimited() public {
        ...
        // Can wrap large amounts
        vm.prank(alice);
        wrapped.wrap{ value: 1000 ether }(alice, "");
        assertEq(wrapped.balanceOf(alice), 1000 ether);
    ...
    function test_liquidityCap_setToZeroDisablesCap() public {
        ...
        // Set a cap
        vm.prank(owner);
        wrapped.setLiquidityCap(10 ether);
        ...
        // Now alice can wrap more
        vm.prank(alice);
        wrapped.wrap{ value: 100 ether }(alice, "");
        assertEq(wrapped.balanceOf(alice), 110 ether);
    }

    function test_liquidityCap_wrapToDifferentRecipient() public {
        NativexD wrapped = NativexD(payable(address(erc20s[0])));
        uint256 cap = 20 ether;
        ...
        // Alice wraps 15 ether to Bob
        vm.prank(alice);
        wrapped.wrap{ value: 15 ether }(bob, "");

        // Bob's wrapped amount is tracked, not Alice's
        assertEq(wrapped.wrappedAmount(bob), 15 ether);
        assertEq(wrapped.wrappedAmount(alice), 0);
        assertEq(wrapped.balanceOf(bob), 15 ether);

        // Bob can't wrap more than 5 ether
        vm.prank(bob);
        vm.expectRevert(abi.encodeWithSelector(INativexD.LiquidityCapExceeded.selector, bob, 15 ether, 6 ether, cap));
        wrapped.wrap{ value: 6 ether }(bob, "");

        // But Alice can wrap up to her cap
        vm.prank(alice);
        wrapped.wrap{ value: 20 ether }(alice, "");
        assertEq(wrapped.wrappedAmount(alice), 20 ether);
    }
}
